...
urlpatterns = [
    ...
    url(r'^boards/(?P<pk>\d+)/topics/(?P<topic_pk>\d+)/posts/(?P<post_pk>\d+)$', views.PostUpdateView.as_view(), name="edit_post"),
    ...
    url(r'^reset/(?P<uidb64>[0-9A-Za-z_\-]+)/(?P<token>[0-9A-Za-z]{1,13}-[0-9A-Za-z]{1,20})/$', 
        auth_views.PasswordResetConfirmView.as_view(template_name="passwrod_reset_confirm.html"), name="passwrod_reset_confirm"),

    url(r'^reset/complete$', auth_views.PasswordResetCompleteView.as_view(template_name="passwrod_reset_complete.html"), 
